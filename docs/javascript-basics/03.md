...
This example makes use of a [template string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) (Made with ` `` `, backticks, which on the same key that <kbd>~</kbd> is). You can insert variables into a template string with `${}`. Also, you can get the body of the HTML document with `document.body`.
