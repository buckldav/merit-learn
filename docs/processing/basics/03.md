...
Notice that the pepper's x position is measured from the left and it y position is measured from the top. Try calling the pepper function a few more times with more different positions to add multiple peppers.
...
Define and call two more functions for toppings. At least one function should have at least one parameter.
